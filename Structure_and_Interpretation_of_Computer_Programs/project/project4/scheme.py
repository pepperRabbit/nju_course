...
# python submit.py --stuid 202220012 --stuname 吴欣瞳
...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    else:   # evaluate call expression
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        procedure = scheme_eval(first, env)
        validate_procedure(procedure)
        arguments = rest.map(lambda k: scheme_eval(k, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"    # bind a name to a val
        rest = expressions.rest
        env.define(target, scheme_eval(rest.first, env))
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # create a lambda procedure and bind to the name
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        validate_form(bindings.first, 2, 2)  # binding.first ( x 5 )
        names = Pair(bindings.first.first, names)  # binding.first.first x
        values = Pair(scheme_eval(bindings.first.rest.first, env), values)  # binding.first.rest.first 5
