...
Operand transExp(node* n) {
    ...
    if(strcmp(n->child->name,"Exp")==0){
        ...
        if(!n->child->sibling) { //Exp->ID
            ...
            transArgs(n->child->sibling->sibling, arg);
            if(strcmp(n->child->val, "write")==0) {
                ...
                p = arg;
                ArgList* tmp = (ArgList*)malloc(sizeof(ArgList)*size);
...
void transArgs(node* n, ArgList list)
{
    list->op = transExp(n->child);
    if(n->child->sibling) { //Args->Exp COMMA Args
        list->next = (ArgList)malloc(sizeof(struct ArgList_));
        transArgs(n->child->sibling->sibling, list->next);
    }
}
