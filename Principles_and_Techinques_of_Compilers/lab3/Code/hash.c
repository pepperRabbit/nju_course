...
{
	//printf("extdeclist\n");
	VarDec(n->child, t, false, false);	// ExrDecList->VarDec
...
FieldList VarDec(node* n, Type t, bool is_struct, bool is_param)
{
	...
	//printf("vardec %s, is struct %d\n", n->child->val, is_struct);
		...
		//if(is_struct){ same...
			...
			return VarDec(n->child, t1, is_struct, is_param);
		//}
        }
        else{//VarDec->ID
			//printf("here\n");
...
{//ParamDec->Specifier VarDec
	//printf("paramdec\n");
	Type t = Specifier(n->child);
	return VarDec(n->child->sibling, t, false, true);
}
...
FieldList Dec(node* n, Type t, bool is_struct)
{
	...
	if(n->child->sibling){//Dec->VarDec ASSIGNOP Exp
		if(is_struct){
			...
			FieldList f = VarDec(n->child, t, is_struct, false);
			if(f){
		...
		if(is_struct){
			return VarDec(n->child, t, is_struct, false);
		}
		else{
			VarDec(n->child, t, is_struct, false);
		}
	}
	return NULL;
}
